...
def handle_validation_exception(error):
    ...
    exception_response["errors"] = (
        error.exception_map
        if error.exception_map
        else {error.error_field_name: error.message}
    )
...
v1_api.add_namespace(ls_auth_ns)
